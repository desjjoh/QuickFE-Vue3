  ...
  <FullContainer>
    <GridBox class="auth-request-page" :data-status="status">
        ...
        <FlexBox direction="column" :gap="6">
          <FlexBox class="auth-request-page__card" direction="column" :gap="2">
            <BlockText element="p" tone="inherit">
              <InlineText class="auth-request-page__eyebrow" size="sm" weight="semibold">
                {{ $t('auth.resendVerificationEmail.success.eyebrow') }}
              </InlineText>
            </BlockText>

            <FlexBox direction="column" :gap="1">
              <BlockText element="h3">
                {{ $t('auth.resendVerificationEmail.success.title') }}
              </BlockText>
              <BlockText element="p">
                {{ $t('auth.resendVerificationEmail.success.message') }}
              </BlockText>
            </FlexBox>
          </FlexBox>
          ...
          <FlexBox justify-content="flex-end">
            <AppLink :href="{ name: 'root' }">
              {{ $t('auth.resendVerificationEmail.actions.backToHome') }}
            </AppLink>
          </FlexBox>
        </FlexBox>
      </template>
      ...
      <template v-else-if="status === 'error'">
        <FlexBox direction="column" :gap="6">
          <FlexBox class="auth-request-page__card" direction="column" :gap="2">
            <BlockText element="p" tone="inherit">
              <InlineText class="auth-request-page__eyebrow" size="sm" weight="semibold">
                {{ $t('auth.resendVerificationEmail.error.eyebrow') }}
              </InlineText>
            </BlockText>

            <FlexBox direction="column" :gap="1">
              <BlockText element="h3">{{
                $t('auth.resendVerificationEmail.error.title')
              }}</BlockText>
              <BlockText element="p">{{
                $t('auth.resendVerificationEmail.error.message')
              }}</BlockText>
            </FlexBox>
          </FlexBox>
          ...
          <FlexBox justify-content="flex-end" :gap="2" wrap="wrap">
            ...
            <BaseButton type="button" variant="soft" @click="status = 'ready'">
              {{ $t('auth.resendVerificationEmail.actions.tryAgain') }}
            </BaseButton>
          </FlexBox>
        </FlexBox>
      </template>
      ...
      <EmailTokenRequest
        ...
        :callback-submit="submitVerificationRequest"
      />
    </GridBox>
  </FullContainer>
</template>
...
const { t } = useI18n()
const { requestEmailVerification } = useAppActions(t)

const status = ref<AuthRequestStatus>('ready')

async function submitVerificationRequest(values: FormValues): Promise<void> {
  try {
